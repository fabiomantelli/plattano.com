      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between px-8 sm:px-8 h-[60px] sm:h-[70px]">
          ...
          <ul className="flex items-center space-x-6 lg:space-x-8 overflow-x-auto scrollbar-hide snap-x snap-mandatory px-2 [&>*]:snap-center">
            {menuItems.map(({ href, label }) => (
              <li key={href}>
                <Link
                  href={href}
                  className="text-black dark:text-white font-medium text-sm sm:text-base dark:hover:text-primary hover:text-white transition duration-150 whitespace-nowrap"
                >
                  {label}
                </Link>
              </li>
        ...
        <div className="ml-auto flex items-center space-x-1 sm:space-x-2 md:space-x-4">
          <div className="hidden md:flex items-center space-x-1 sm:space-x-2">
            <Link href="https://www.instagram.com/plattanotechnologies.us/" target="_blank" aria-label="Instagram">
              <FaInstagram className="h-5 w-5 dark:text-primary text-black hover:opacity-80 transition-opacity" />
            </Link>
            <Link href="https://www.linkedin.com/company/plattano-technologies/" target="_blank" aria-label="LinkedIn">
          ...
          {mounted && (
            <>
              <Link href="https://instagram.com/plattano.us" target="_blank" aria-label="Instagram">
                <FaInstagram className={`h-6 w-6 ${resolvedTheme === 'light' ? 'text-black' : 'text-[var(--color-primary)]'}`} />
              </Link>
              <Link href="https://www.linkedin.com/company/plattano-technologies/" target="_blank" aria-label="LinkedIn">
                <FaLinkedin className={`h-6 w-6 ${resolvedTheme === 'light' ? 'text-black' : 'text-[var(--color-primary)]'}`} />
              </Link>
            </>
          )}
          <ThemeToggle />
